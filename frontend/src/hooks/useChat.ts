  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      if (state.mode === 'pdf') {
        ...
        const sessionId = localStorage.getItem('pdfchat-session-id') || `session-${Date.now()}`;
        if (!localStorage.getItem('pdfchat-session-id')) {
          localStorage.setItem('pdfchat-session-id', sessionId);
        }
